...
const DispatchedOrder = () => {
  ...
  const {
    ...
    loading,
    error,
  } = useFetchOrdersByStatus('dispatched');

  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - dispatched.length) : 0;
        ...
        <TableContainer>
          {loading ? (
            ...
            <Table sx={{ whiteSpace: 'nowrap' }}>
              ...
              <TableBody>
                {(rowsPerPage > 0
                  ? dispatched.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                  : dispatched
                ).map((row) => (
                  <TableRow key={row.id}>
